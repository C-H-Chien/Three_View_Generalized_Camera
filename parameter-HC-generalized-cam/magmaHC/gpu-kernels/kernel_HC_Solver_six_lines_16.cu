#ifndef kernel_HC_Solver_six_lines_16_cu
#define kernel_HC_Solver_six_lines_16_cu
// ============================================================================
// GPU-HC solver for six lines 16x16 generalized camera model problem
//
// Modifications
//    Chiang-Heng Chien  22-11-23:   Initially created. 
//    Copied and edited from kernel_HC_Solver_six_lines_generalized_cam.cu file.
//
...
//> device function
#include "../gpu-idx-evals/dev-eval-indxing-six_lines_16.cuh"
...
namespace magmaHCWrapper {
  ...
  __global__ void
  homotopy_continuation_solver_six_lines_16(
    magma_int_t ldda,
  ...
  )
  {
    ...
    d_cgesvB[tx] = (t0 >= 1.0 || (1.0-t0 <= 0.0000001)) ? MAGMA_C_MAKE(1.0, t0) : MAGMA_C_MAKE(t0, delta_t);
  ...
  )
  {
    ...
    e = cudaLaunchKernel((void*)homotopy_continuation_solver_six_lines_16
                         < 16, 157, 50, 4, 4, 6, 3, 54, 4 >, 
                         grid, threads, kernel_args, shmem, my_queue->cuda_stream());

    gpu_time = magma_sync_wtime( my_queue ) - gpu_time;
    if( e != cudaSuccess ) {
        printf("cudaLaunchKernel of homotopy_continuation_solver_six_lines_16 is not successful!\n");
    }
