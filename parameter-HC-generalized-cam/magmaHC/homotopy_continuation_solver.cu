...
namespace magmaHCWrapper {
    ...
    problem_params* pp, std::string hc_problem, std::ofstream &track_sols_file)
  {
    ...
    if (hc_problem == "six_lines_6x6") {
      std::cout<<"Solving six lines problem under a generalized camera model ..."<<std::endl<<std::endl;
      gpu_time = kernel_HC_Solver_six_lines_6x6(N, batchCount, ldda, my_queue, d_startSols_array, d_Track_array, d_cgesvA_array, d_cgesvB_array, 
                                                d_Hx_idx, d_Ht_idx, d_phc_coeffs_Hx, d_phc_coeffs_Ht, pp->numOfCoeffsFromParams);
    }
    else if (hc_problem == "six_lines_16") {
      std::cout<<"Solving six lines 16x16 problem under a generalized camera model ..."<<std::endl<<std::endl;
      gpu_time = kernel_HC_Solver_six_lines_16(N, batchCount, ldda, my_queue, d_startSols_array, d_Track_array, d_cgesvA_array, d_cgesvB_array, 
                                               d_Hx_idx, d_Ht_idx, d_phc_coeffs_Hx, d_phc_coeffs_Ht, pp->numOfCoeffsFromParams);
    }
    else if (hc_problem == "3views_4pts") {
      std::cout<<"Solving 3 Views with 4 Points Problem ..."<<std::endl<<std::endl;
      gpu_time = kernel_HC_Solver_3views_4pts(N, batchCount, ldda, my_queue, d_startSols_array, d_Track_array, d_cgesvA_array, d_cgesvB_array, 
                                              d_Hx_idx, d_Ht_idx, d_phc_coeffs_Hx, d_phc_coeffs_Ht, pp->numOfCoeffsFromParams);
    }
