#ifndef KERNEL_HC_solver_generalized_3views_3orientedpoints_CU
#define KERNEL_HC_solver_generalized_3views_3orientedpoints_CU
// =============================================================================================================
// GPU homotopy continuation solver for Generalized Three-View Relative Pose Problem from 4 points
//
// Modifications
//    Chiang-Heng Chien  25-07-01:   Build on top of generalized_3views_4pts
//
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-generalized_3views_3orientedpoints.cuh"
#include "../dev-cgesv-batched-small.cuh"

template< int Num_Of_Vars, int Num_Of_Params, \
          int Num_of_Coeffs_from_Params, int Num_Of_Terms_Per_Coeff, int Num_Of_P2C_Params_Per_Term, int Num_Of_Const_Coeffs_in_dHdt, \
...
__global__ void
HC_solver_generalized_3views_3orientedpoints(
  ...
  magmaComplex*             d_startParams,
  magmaComplex*             d_targetParams,
  ...
  const int* __restrict__   d_P2C_Index,
  bool*                     d_is_GPU_HC_Sol_Converge, 
  bool*                     d_is_GPU_HC_Sol_Infinity,
  magmaComplex*             d_Debug_Purpose                 //> optional
)
{
  ...
  const int* __restrict__ d_dHdx_idx = d_dHdx_indices;
  const int* __restrict__ d_dHdt_idx = d_dHdt_indices;
  const int* __restrict__ d_P2C_idx  = d_P2C_Index;
...
//> shared memory declarations
  ...
  magmaComplex *s_coeffs_dHdx        = sx                       + Num_Of_Vars;
  magmaComplex *s_coeffs_dHdt        = s_coeffs_dHdx            + (Num_of_Coeffs_from_Params+1);
  FP_type* dsx                       = (FP_type*)(s_coeffs_dHdt + (Num_of_Coeffs_from_Params+1));
  ...
  //> CH: TODO: ADD START PARAMS, TARGET PARAMS, PARAMS_DIFF, ETC HERE...
  ...
  SHARED_ALLOC(magmaComplex, s_coeffs_dHdx, Num_of_Coeffs_from_Params+1);
  SHARED_ALLOC(magmaComplex, s_coeffs_dHdt, Num_of_Coeffs_from_Params+1);
  ...
  if (tx == 0) {
    ...
    s_param_homotopy[Num_Of_Params]           = MAGMA_MAKE_COMPLEX(1.0, 0.0);
    s_coeffs_dHdx[Num_of_Coeffs_from_Params]  = MAGMA_MAKE_COMPLEX(1.0, 0.0);
    s_coeffs_dHdt[Num_of_Coeffs_from_Params]  = MAGMA_MAKE_COMPLEX(1.0, 0.0);
    sipiv[Num_Of_Vars]                        = 0;
  }
  magmablas_syncwarp();
  ...
  for (int step = 0; step <= hc_max_steps; step++) {
    if (t0 < 1.0 && (1.0-t0 > 0.0000001)) {
      ...
      for (int rk_step = 0; rk_step < 4; rk_step++ ) {
        ...
        eval_Jacobian_Ht< dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset >( tx, s_track, r_cgesvB, d_dHdt_idx, s_coeffs_dHdt );
      ...
      for(int i = 0; i < hc_max_xorrection_steps; i++) {
        ...
        eval_Jacobian_Hx< Num_Of_Vars, dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset >( tx, s_track, r_cgesvA, d_dHdx_idx, s_coeffs_dHdx );
        eval_Homotopy< dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset >( tx, s_track, r_cgesvB, d_dHdt_idx, s_coeffs_dHdx );
...
real_Double_t
kernel_HC_Solver_generalized_3views_3orientedpoints(
...
)
{
  ...
  const int num_of_vars               = 15;
  const int num_of_tracks             = 840;
  const int num_of_params             = 54;
  const int num_of_coeffs_from_params = 330;
  const int dHdx_Max_Terms            = 40;
  const int dHdx_Max_Parts            = 5;
  const int dHdt_Max_Terms            = 100;
  const int dHdt_Max_Parts            = 6;
  const int max_order_of_t            = 6;

  //> newly added for computing polynomial coefficients from system parameters
  const int num_of_terms_per_coeff      = 12;
  const int num_of_p2c_params_per_term  = 6+1;
  const int num_of_const_coeffs_in_dHdt = 48;
  ...
  shmem += (num_of_params+1)             * sizeof(magmaComplex);   // s_startParams
  // shmem += (num_of_params+1)             * sizeof(magmaComplex);   // s_targetParams
  shmem += (num_of_params+1)             * sizeof(magmaComplex);   // s_diff_params
  shmem += (num_of_params+1)             * sizeof(magmaComplex);   // s_param_homotopy
  shmem += (num_of_coeffs_from_params+1) * sizeof(magmaComplex);   // s_coeffs_dHdx
  shmem += (num_of_coeffs_from_params+1) * sizeof(magmaComplex);   // s_coeffs_dHdt
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_generalized_3views_3orientedpoints \
                                    < num_of_vars, num_of_params, num_of_coeffs_from_params, num_of_terms_per_coeff, \
                                      num_of_p2c_params_per_term, num_of_const_coeffs_in_dHdt, \
  ...
  e = cudaLaunchKernel((void*)HC_solver_generalized_3views_3orientedpoints \
                        < num_of_vars, num_of_params, num_of_coeffs_from_params, num_of_terms_per_coeff, \
                          num_of_p2c_params_per_term, num_of_const_coeffs_in_dHdt, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset >, \
